...
class PedidosLibros():
    ...
    def seccion_uno(self, frame_datos):
        ...
        pedido_boton.grid(column=0, row=13, padx=30, pady=[5,10], sticky='w', columnspan=2)
    ...
    def pedido(self):
        ...
        if cantidad_pedida > 0 and len(diccionario_libro['values']) != 0 and len(diccionario_alumno['values']) != 0:
            hoy = date.today()
            situacion = 'prestado'
            observacion = 'ninguna'
            tipo = 'libro'
